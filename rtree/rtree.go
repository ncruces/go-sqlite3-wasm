...
func New(v0 Xenv) *Module {
	...
	copy((*m.memory)[uint32(m.___memory_base+i32(3104)):], data1)
...
func (m *Module) _rtreeOpen(v0, v1 int32) int32 {
	var v2 int32
	t0 := m._sqlite3_malloc64(i64(248))
	v2 = t0
	var p1 int32
	if v2 != 0 {
		memory_zero(*m.memory, uint32(v2), uint32(i32(248)))
...
l1:
	...
	v1 = t10
	memory_zero(*m.memory, uint32(v0), uint32(i32(248)))
	...
	var v4, v5, v6 int32
	{
		...
		v4 = v0 + i32(80)
		{
			...
			l2:
				...
				t12 := m._sqlite3_blob_bytes(v5)
				if t11 != t12 {
					v5 = i32(0)
					goto l4
				}
				t13 := int32(load32((*m.memory)[int64(uint32(v0))+16:]))
				t14 := m._sqlite3_malloc64(int64(uint32(t13 + i32(32))))
				v6 = t14
				if v6 == 0 {
					v5 = i32(7)
					goto l4
				...
				store64((*m.memory)[int64(uint32(v6))+8:], uint64(v1))
				t15 := v6
				v4 = v6 + i32(32)
				store32((*m.memory)[int64(uint32(t15))+24:], uint32(v4))
				t16 := int32(load32((*m.memory)[int64(uint32(v0))+68:]))
				store32((*m.memory)[int64(uint32(v0))+68:], uint32(t16+i32(1)))
				t17 := int32(load32((*m.memory)[int64(uint32(v0))+80:]))
				t18 := int32(load32((*m.memory)[int64(uint32(v0))+16:]))
				t19 := m._sqlite3_blob_read(t17, v4, t18, i32(0))
				v5 = t19
				v4 = v6
				goto l4
			}
	...
	l4:
		{
			...
			t22 := v0
			v6 = t21
			t23 := int32((*m.memory)[int64(uint32(v6))+1])
			t24 := int32((*m.memory)[uint32(v6)])
			v6 = t23 | t24<<8
			store32((*m.memory)[int64(uint32(t22))+32:], uint32(v6))
			p25 := i32(0)
		...
		{
			{
				...
				v6 = t29
				t30 := int32((*m.memory)[int64(uint32(v6))+3])
				t31 := int32((*m.memory)[int64(uint32(v6))+2])
				if t28 < t30|t31<<8 {
					goto l7
				}
				goto l8
...
const (
	data0 = "}\x00%z%s%z\x00In RTree %s.%s:\n%z\x00geopoly\x00geopoly_bbox\x00geopoly_group_bbox\x00geopoly_ccw\x00Auxiliary rtree columns must be last\x00Dimension %d of cell %d on node %lld is corrupt\x00geopoly_contains_point\x00%_parent\x00Dimension %d of cell %d on node %lld is corrupt relative to parent\x00,%.*s\x00UNIQUE constraint failed: %s.%s\x00,%s\x00 %s\x00geopoly_regular\x00geopoly_overlap\x00INSERT INTO\"%w\".\"%w_rowid\"(rowid,nodeno)VALUES(?1,?2)ON CONFLICT(rowid)DO UPDATE SET nodeno=excluded.nodeno\x00CREATE TABLE \"%w\".\"%w_rowid\"(rowid INTEGER PRIMARY KEY,nodeno\x00geopoly_json\x00_shape does not contain a valid polygon\x00geopoly_within\x00main\x00fullscan\x00geopoly_xform\x00ok\x00rtreecheck\x00rtreedepth\x00geopoly_svg\x00geopoly_debug\x00RtreeMatchArg\x00%c%g,%g\x00 %g\x00PRAGMA %Q.page_size\x00Node %lld missing from database\x00CREATE TABLE x(_shape\x00Too many columns for a geopoly table\x00Mapping (%lld -> %lld) missing from %s table\x00Too many columns for an rtree table\x00Too few columns for an rtree table\x00Wrong number of columns for an rtree table\x00Schema corrupt or not an rtree\x00rtreenode\x00_node\x00{%lld\x00Wrong number of entries in %%%s table - expected %lld, actual %lld\x00%_rowid\x00,a%d\x00a%d=?%d\x00geopoly_blob\x00data\x00geopoly_area\x00[%!g,%!g]]\x00[\x00,%.*s INT\x00CREATE TABLE x(%.*s INT\x00SELECT * FROM %Q.%Q\x00,%.*s REAL\x00SELECT data FROM %Q.'%q_node' WHERE nodeno=?\x00></polyline>\x00<polyline points=\x00CREATE TABLE \"%w\".\"%w_parent\"(nodeno INTEGER PRIMARY KEY,parentnode);\x00);CREATE TABLE \"%w\".\"%w_node\"(nodeno INTEGER PRIMARY KEY,data);\x00DROP TABLE '%q'.'%q_node';DROP TABLE '%q'.'%q_rowid';DROP TABLE '%q'.'%q_parent';\x00ALTER TABLE %Q.'%q_node'   RENAME TO \"%w_node\";ALTER TABLE %Q.'%q_parent' RENAME TO \"%w_parent\";ALTER TABLE %Q.'%q_rowid'  RENAME TO \"%w_rowid\";\x00rtree_i32\x00sqlite_stat1\x00SELECT parentnode FROM %Q.'%q_parent' WHERE nodeno=?1\x00SELECT nodeno FROM %Q.'%q_rowid' WHERE rowid=?1\x00SELECT * FROM \"%w\".\"%w_rowid\" WHERE rowid=?1\x00SELECT parentnode FROM '%q'.'%q_parent' WHERE nodeno = ?1\x00DELETE FROM '%q'.'%q_parent' WHERE nodeno = ?1\x00DELETE FROM '%q'.'%q_node' WHERE nodeno = ?1\x00SELECT nodeno FROM '%q'.'%q_rowid' WHERE rowid = ?1\x00DELETE FROM '%q'.'%q_rowid' WHERE rowid = ?1\x00SELECT length(data) FROM '%q'.'%q_node' WHERE nodeno = 1\x00[%!g,%!g],\x00Node %lld is too small (%d bytes)\x00Node %lld is too small for cell count of %d (%d bytes)\x00rtree constraint failed: %s.(%s<=%s)\x00Found (%lld -> %lld) in %s table, expected (%lld -> %lld)\x00a%d=coalesce(?%d,a%d)\x00Rtree depth out of range (%d)\x00INSERT OR REPLACE INTO '%q'.'%q_parent' VALUES(?1, ?2)\x00INSERT OR REPLACE INTO '%q'.'%q_node' VALUES(?1, ?2)\x00INSERT OR REPLACE INTO '%q'.'%q_rowid' VALUES(?1, ?2)\x00INSERT INTO \"%w\".\"%w_node\"VALUES(1,zeroblob(%d))\x00wrong number of arguments to function rtreecheck()\x00Invalid argument to rtreedepth()\x00SELECT count(*) FROM %Q.'%q%s'\x00 %g,%g'\x00SELECT * FROM %Q.'%q_rowid'\x00SELECT stat FROM %Q.sqlite_stat1 WHERE tbl = '%q_rowid'\x00undersize RTree blobs in \"%q_node\"\x00UPDATE \"%w\".\"%w_rowid\"SET \x00\n"
	data1 = "\x04\x00\x00\x00\n\x00\x00\x00\v\x00\x00\x00\f\x00\x00\x00\rm\x03\x00\x00I\x03\x00\x00P\x00\x00\x00\xaa\x04\x00\x00t\x04\x00\x00\x00\x00\x00\x00r\t\x00\x00}\a\x00\x00\xaa\a\x00\x00\xa7\t\x00\x00\xde\a\x00\x00\x14\a\x00\x00;\t\x00\x00N\a\x00\x00\xe5\x03\x00\x00\x01\x01\x00\x005\x04\x00\x00\x00\x00\x00\x00\x1e\x00\x00\x00\x01\x01\x00\x00Z\x04\x00\x00\x1f\x00\x00\x00\x01\x01\x00\x00H\x04\x00\x00 \x00\x00\x00\x01\x01\x00\x00\x00\x02\x00\x00!\x00\x00\x00\xff\x01\x00\x00y\x02\x00\x00\t\x00\x00\x00\x02\x01\x00\x005\x02\x00\x00\"\x00\x00\x00\x03\x01\x00\x00\xa5\x00\x00\x00\b\x00\x00\x00\x02\x01\x00\x00F\x01\x00\x00#\x00\x00\x00\x01\x00\x00\x00\x85\x02\x00\x00$\x00\x00\x00\x01\x01\x00\x00$\x00\x00\x00%\x00\x00\x00\a\x01\x00\x00R\x02\x00\x00&\x00\x00\x00\x04\x01\x00\x006\x01\x00\x00'\x00\x00\x00\x01\x01\x00\x00D\x00\x00\x00\x03\x00\x00\x00(\x00\x00\x00)\x00\x00\x00*\x00\x00\x00\r\x00\x00\x00\x0e\x00\x00\x00\x0f\x00\x00\x00\x10\x00\x00\x00+\x00\x00\x00\x12\x00\x00\x00\x13\x00\x00\x00,\x00\x00\x00\x15\x00\x00\x00-\x00\x00\x00\x17\x00\x00\x00\x18\x00\x00\x00\x18\x00\x00\x00\x18\x00\x00\x00.\x00\x00\x00\x1a\x00\x00\x00\x1b\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x1c\x00\x00\x00\x1d\x00\x00\x00"
)
